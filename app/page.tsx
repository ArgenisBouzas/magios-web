...
export default function Home() {
  return (
    <div className="min-h-screen bg-[#0a0c0e] text-[#c4aa7d]">
      ...
      <nav className="bg-[#1a1f23] border-y-2 border-[#8b6f4c] py-2 overflow-x-auto">
        <ul className="flex justify-start sm:justify-center space-x-4 sm:space-x-8 text-[#c4aa7d] px-4 min-w-max sm:min-w-0">
          {[
            { nombre: 'Inicio', ruta: '/' },
            ...
            <li key={item.nombre}>
              <Link 
                href={item.ruta}
                className="px-2 sm:px-4 py-1 sm:py-2 hover:text-[#f0d9b5] hover:bg-[#2a2f33] transition-all duration-300 border-b-2 border-transparent hover:border-[#8b6f4c] uppercase tracking-wider text-xs sm:text-sm font-semibold whitespace-nowrap"
              >
                {item.nombre}
              </Link>
            </li>
